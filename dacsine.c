// MCP4822 12 bit dac sine wave demo
// Version 1.0  - 11/08/2013
// Version History:
// 1.0 - Initial Release
//
//
// Usage: setvoltage(channel1, channel2) to change to set new output value between 0 and 4096 on each output
//
// Compile:  gcc dacsine.c -std=c99 -o dacsine
//Run: ./dacsine
...
void setvoltage(int voltA, int voltB) {
	...
	// uncomment following lines to output on second channel
	/*
...
int main(int argc, char *argv[])
{
	...
	while(1) {
		...
		for (i=0; i<512; ++i){
			...
			setvoltage(DACLookup[i],DACLookup[i]);
		}
		i = 0;	
